    ...
    <>
      ...
      <aside className={cn('spatial-nav', open && 'spatial-nav-open')} aria-label="Spatial navigation">
        <div className="spatial-nav-header">
          ...
          <div><strong>NEXORA</strong><small>{role}</small></div>
        ...
        <nav className="spatial-links">
          {ITEMS.map(({ label, path, icon: Icon }) => {
            ...
            return <NavLink key={path} to={destination} onClick={onClose} className={({ isActive }) => cn('spatial-link', isActive && 'spatial-link-active')}>
              <Icon size={16} /><span>{label}</span><i />
            </NavLink>;
          })}
          {state.role === 'admin' && <NavLink to="/settings" onClick={onClose} className="spatial-link"><Users size={16} /><span>Settings</span><i /></NavLink>}
        </nav>
